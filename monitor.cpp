...
void RebuildListView() {
  ...
  for (auto &grp : g_groups) {
    ...
    // Parent row - use Unicode triangles
  ...
  int newTop = ListView_GetTopIndex(g_hList);
  if (topIdx > newTop && topIdx < idx) {
    RECT rc;
    if (ListView_GetItemRect(g_hList, 0, &rc, LVIR_BOUNDS)) {
      ListView_Scroll(g_hList, 0, (topIdx - newTop) * (rc.bottom - rc.top));
    }
  }
  ...
  wchar_t sb[200];
  swprintf_s(sb,
             L"  %d processes   \u2502   RAM: %s   \u2502   CPU: %.1f%%   \u2502  "
             L" Refresh: %dms",
             idx, FormatBytes(totRam).c_str(), totCpu, g_refreshMs);
  g_statusText = sb;
  ...
  case CDDS_ITEMPREPAINT | CDDS_SUBITEM: {
    ...
    if (cd->iSubItem == 2 || cd->iSubItem == 3) {
      ...
      if (cd->iSubItem == 2) { // CPU bar
        ...
        if (v < 15)      barCol = RGB(40, 140, 100);
        else if (v < 40) barCol = RGB(180, 150, 40);
        else if (v < 70) barCol = RGB(220, 120, 30);
        else             barCol = RGB(220, 60, 60);
      } else { // Memory bar
        double val = _wtof(buf);
        if (wcsstr(buf, L"GB"))      val *= 1073741824.0;
        else if (wcsstr(buf, L"MB")) val *= 1048576.0;
        else if (wcsstr(buf, L"KB")) val *= 1024.0;
...
void DrawDashboard(HWND hWnd, HDC dc) {
    ...
    struct { const wchar_t* label; wchar_t value[64]; } stats[] = {
        { L"CPU Cores", {} }, { L"Threads", {} }, { L"Uptime", {} }
    };
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        DrawTextW(dc, stats[i].label, -1, &lr, DT_LEFT | DT_VCENTER | DT_SINGLELINE);
        SetTextColor(dc, C::TEXT);
        RECT vr = { cardX + 120, iy, cardX + cardW - 10, iy + 22 };
        DrawTextW(dc, stats[i].value, -1, &vr, DT_RIGHT | DT_VCENTER | DT_SINGLELINE);
    }

    // === Graph helper ===
    auto DrawGraph = [&](const std::deque<double>& data, int y,
                         const wchar_t* title, COLORREF lineCol, COLORREF fillCol) {
        DrawCard(y, 160);
    ...
    DrawGraph(g_cpuHistory, dr.top + 150, L"CPU Usage",
              RGB(80, 200, 255), RGB(20, 50, 70));
    DrawGraph(g_ramHistory, dr.top + 320, L"RAM Usage",
              RGB(170, 120, 255), RGB(40, 25, 65));
    ...
    for (int i = 0; i < std::min(5, (int)sorted.size()); ++i) {
        ...
        DeleteObject(bbr);
        RECT barFill = barBg;
        barFill.right = barFill.left + (int)((barFill.right - barFill.left) * ratio);
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT msg, WPARAM wp, LPARAM lp) {
  ...
  case WM_CREATE: {
    ...
    g_hFontBd = CreateFontW(16, 0, 0, 0, FW_SEMIBOLD, 0, 0, 0, DEFAULT_CHARSET,
                            OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS,
    ...
    g_hSearch = CreateWindowExW(
        ...
        20, HDR_H + 10, 280, 28, hWnd, (HMENU)IDC_SEARCH, nullptr, nullptr);
    ...
    g_hRefresh = CreateWindowExW(
        0, L"BUTTON", L"\u21BB  Refresh", WS_CHILD | WS_VISIBLE | BS_OWNERDRAW,
        W - 165, HDR_H + 10, 140, 28, hWnd, (HMENU)IDC_REFRESH, nullptr, nullptr);
    ...
    g_hList = CreateWindowExW(
        ...
        0, listTop, W, H - listTop - STATUS_H,
        hWnd, (HMENU)IDC_LIST, nullptr, nullptr);
  ...
  case WM_SIZE: {
    ...
    if (g_hRefresh)
      SetWindowPos(g_hRefresh, nullptr, W - 165, HDR_H + 10, 140, 28, SWP_NOZORDER);
    if (g_hUnit)
      SetWindowPos(g_hUnit, nullptr, W - 315, HDR_H + 10, 140, 28, SWP_NOZORDER);
    if (g_hSpeed)
      SetWindowPos(g_hSpeed, nullptr, W - 465, HDR_H + 10, 140, 28, SWP_NOZORDER);
  ...
  case WM_DRAWITEM: {
    ...
    if (dis->CtlID == IDC_REFRESH || dis->CtlID == IDC_UNIT || dis->CtlID == IDC_SPEED) {
      ...
      SetTextColor(dis->hDC,
                   disabled ? RGB(70, 72, 90) : C::TEXT);
      SelectObject(dis->hDC, g_hFontSm);
  ...
  case WM_COMMAND:
    ...
    if (LOWORD(wp) == IDC_SPEED) {
      if (g_refreshMs == 500) g_refreshMs = 1000;
      else if (g_refreshMs == 1000) g_refreshMs = 2000;
      else if (g_refreshMs == 2000) g_refreshMs = 5000;
      else g_refreshMs = 500;
      
      wchar_t buf[32];
      if (g_refreshMs == 500) wcscpy_s(buf, L"Speed: 0.5s");
      else swprintf_s(buf, L"Speed: %ds", g_refreshMs / 1000);
...
int WINAPI WinMain(HINSTANCE hInst, HINSTANCE, LPSTR, int nShow) {
  ...
  HWND hWnd = CreateWindowExW(0, L"ProcessMonitorClass", L"HALE \u2014 System Vitality Monitor",
                              WS_OVERLAPPEDWINDOW | WS_CLIPCHILDREN | WS_CLIPSIBLINGS,
                              CW_USEDEFAULT, CW_USEDEFAULT, 1100, 750, nullptr, nullptr, hInst, nullptr);
